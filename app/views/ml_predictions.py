...
def render():
    ...
    with tab1:
        ...
        if pred_df.empty:
            st.warning("No predictions available. Run Data Management → Initialize / Reprocess All Data.")
            st.dataframe(data["institutions"].head(5), use_container_width=True)
        else:
            st.dataframe(pred_df, use_container_width=True, hide_index=True)
            if "accreditation_readiness" in pred_df.columns:
                section_panel("Accreditation Readiness Distribution")
    ...
    with tab2:
        ...
        if st.button("Generate Prediction", type="primary", key=widget_key("ml", "predict_btn")):
            ml = MLService()
            if not ml.load_models():
                st.warning("Models not trained. Displaying illustrative output.")
                m1, m2, m3 = st.columns(3)
                m1.metric("Performance Score", "72.50")
                ...
                if result:
                    ...
                    m1, m2, m3 = st.columns(3)
                    m1.metric("Performance Score", f"{result.get('predicted_performance_score', 0):.2f}")
    ...
    st.caption("Institutional Performance Analytics | Machine Learning Intelligence Module")
